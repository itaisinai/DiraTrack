...
import { SourceSelectionDialog, type SourceMetadata } from "@/components/source-selection-dialog";
...
function ProjectPage() {
  ...
  async function startResearch(selectedKeys: string[], externalDataConsent: boolean) {
    setStarting(true); setError("");
    try {
      const response = await fetch(`/api/projects/${encodedSlug}/research-runs`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sourceKeys: selectedKeys, externalDataConsent }),
      });
  ...
  return <AppShell>
    ...
    {data && <>
      <header className="mb-8 flex flex-wrap items-start justify-between gap-4"><div><Link href="/" className="text-sm font-medium text-[var(--primary)]">→ כל הפרויקטים</Link><p className="mb-2 mt-5 text-sm text-[var(--muted)]">{[data.project.city, data.project.developer].filter(Boolean).join(" · ")}</p><h1 className="text-3xl font-bold">{data.project.name}</h1></div><button onClick={() => setShowSourceSelection(true)} disabled={starting} className="rounded-lg bg-[var(--primary)] px-5 py-3 font-semibold text-white disabled:opacity-60">הפעלת מחקר חדש</button></header>
      {error && <p className="mb-5 rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}
      <StageTracker currentStage={data.project.stage} />
      <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Summary title="הסטטוס הנוכחי" body={data.project.operationalStatus}/><Summary title="מה השתנה לאחרונה" body={runs[0] ? researchStatus(runs[0].status) : "טרם הופעל מחקר"}/><Summary title="מזהים שנשמרו" body={`${data.identifiers.length} מזהים`}/><SummaryLink title="ספריית המסמכים" body="צפייה במסמכים שהורדו" href={`/projects/${encodedSlug}/documents`}/></div>
      ...
      {showSourceSelection && <SourceSelectionDialog projectSlug={encodedSlug} sources={sources} onStart={startResearch} onClose={() => setShowSourceSelection(false)} isStarting={starting} />}
    </>}
  </AppShell>;
}
...
function SummaryLink({ title, body, href }: { title: string; body: string; href: string }) { return <Link href={href} className="rounded-xl border border-[var(--border)] border-r-4 border-r-blue-400 bg-white p-5 transition-shadow hover:shadow-lg"><h2 className="text-sm font-medium text-[var(--muted)]">{title}</h2><p className="mt-3 font-semibold text-[var(--primary)]">{body} →</p></Link>; }
