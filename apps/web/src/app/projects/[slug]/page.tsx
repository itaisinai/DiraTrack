...
function ProjectPage() {
  ...
  async function startResearch() {
    ...
    try {
      const response = await fetch(`/api/projects/${encodedSlug}/research-runs`, {
        ...
        body: JSON.stringify({ sourceKeys: ["discounted-housing", "asia-cyrus"], externalDataConsent }),
      });
  ...
  return <AppShell>
    ...
    {data && <>
      ...
      {showResearchConfirmation && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4" role="presentation" onMouseDown={() => setShowResearchConfirmation(false)}><section role="dialog" aria-modal="true" aria-labelledby="research-confirmation-title" onMouseDown={(event) => event.stopPropagation()} className="w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl sm:p-8"><h2 id="research-confirmation-title" className="text-2xl font-bold">אישור מחקר במקורות חיצוניים</h2><p className="mt-3 text-[var(--muted)]">המחקר יבדוק את אתר „דירה בהנחה” ואת אתר אסיה סיירוס. מספר ההגרלה יישלח או יוצג לחיפוש באתר הרשמי; אתר היזם יקבל את שם הפרויקט, העיר והמזהים ששמרת.</p><div className="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><strong>חשוב:</strong> המקורות החיצוניים עשויים לתעד את השאילתות. האתר הרשמי עשוי לדרוש פעולה ידנית. תוצאות שיימצאו יסומנו „דורש בדיקה” ולא ישנו עובדות בפרויקט.</div><label className="mt-5 flex items-start gap-3 text-sm"><input type="checkbox" checked={externalDataConsent} onChange={(event) => setExternalDataConsent(event.target.checked)} className="mt-1"/><span>אני מאשר/ת להשתמש בנתוני החיפוש המפורטים מול שני המקורות לצורך המחקר.</span></label>{error && <p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}<div className="mt-7 flex justify-end gap-3"><button onClick={() => setShowResearchConfirmation(false)} className="rounded-lg border border-[var(--border)] px-5 py-3 font-semibold">ביטול</button><button onClick={startResearch} disabled={!externalDataConsent || starting} className="rounded-lg bg-[var(--primary)] px-5 py-3 font-semibold text-white disabled:opacity-50">{starting ? "מתחיל מחקר…" : "אישור והתחלת מחקר"}</button></div></section></div>}
    </>}
  </AppShell>;
}
