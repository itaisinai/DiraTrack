...
function ProjectPage() {
  ...
  async function startResearch() {
    ...
    try {
      const response = await fetch(`/api/projects/${encodedSlug}/research-runs`, {
        ...
        body: JSON.stringify({ externalDataConsent }),
      });
  ...
  return <AppShell>
    ...
    {data && <>
      ...
      {showResearchConfirmation && <div className=”fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4” role=”presentation” onMouseDown={() => setShowResearchConfirmation(false)}><section role=”dialog” aria-modal=”true” aria-labelledby=”research-confirmation-title” onMouseDown={(event) => event.stopPropagation()} className=”w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl sm:p-8”><h2 id=”research-confirmation-title” className=”text-2xl font-bold”>אישור מחקר במקורות</h2><p className=”mt-3 text-[var(--muted)]”>המחקר החי יבדוק מספר מקורות. חלק מהמקורות נבדקים אוטומטית בעוד אחרים דורשים פעולה ידנית.</p><div className=”mt-4 space-y-2 text-sm”><p><strong>מקורות אוטומטיים:</strong></p><ul className=”mr-5 list-disc space-y-1 text-[var(--muted)]”><li>אתר אסיה סיירוס - יקבל שאילתות המכילות את שם הפרויקט, העיר והמזהים (הגרלה, גוש, חלקה וכו׳)</li></ul><p className=”mt-3”><strong>מקורות ידניים:</strong></p><ul className=”mr-5 list-disc space-y-1 text-[var(--muted)]”><li>דירה בהנחה - תקבל/י הנחיות לחיפוש ידני באתר הרשמי</li></ul></div><div className=”mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900”><strong>חשוב:</strong> האתרים החיצוניים עשויים לתעד את השאילתות. תוצאות שיימצאו יסומנו „דורש בדיקה” ולא ישנו עובדות בפרויקט.</div><label className=”mt-5 flex items-start gap-3 text-sm”><input type=”checkbox” checked={externalDataConsent} onChange={(event) => setExternalDataConsent(event.target.checked)} className=”mt-1”/><span>אני מאשר/ת לשלוח נתוני חיפוש למקורות אוטומטיים ומסכים/ה לבצע בדיקות ידניות במקורות רשמיים.</span></label>{error && <p className=”mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700”>{error}</p>}<div className=”mt-7 flex justify-end gap-3”><button onClick={() => setShowResearchConfirmation(false)} className=”rounded-lg border border-[var(--border)] px-5 py-3 font-semibold”>ביטול</button><button onClick={startResearch} disabled={!externalDataConsent || starting} className=”rounded-lg bg-[var(--primary)] px-5 py-3 font-semibold text-white disabled:opacity-50”>{starting ? “מתחיל מחקר…” : “אישור והתחלת מחקר”}</button></div></section></div>}
    </>}
  </AppShell>;
}
