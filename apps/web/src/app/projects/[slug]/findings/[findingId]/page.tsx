...
import { DocumentDownloadDialog } from "@/components/document-download-dialog";
...
function FindingPage() {
  ...
  const canDownloadAsDocument = finding?.sourceUrl && finding.sourceUrl.startsWith("https://");
  ...
  return (
    <AppShell>
      ...
      {finding && (
        <>
          <Link
            href={`/projects/${encodedSlug}/research/${encodeRouteSegment(finding.researchRunId)}`}
          ...
          <header className="mt-6 rounded-xl border border-[var(--border)] bg-white p-6 sm:p-8">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="text-sm text-[var(--muted)]">
                  {finding.source.name} · {sourceCategoryLabel(finding.source.category)}
                </p>
            ...
            <div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
              הממצא מבוסס על תוצאת חיפוש במקור חי. התאמה בחיפוש אינה מוכיחה שהעמוד מתייחס לפרויקט,
              ולכן הוא לא ישנה שום פרט בפרויקט ללא בדיקה נוספת.
            </div>
          </header>

          <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_22rem]">
            <section className="rounded-xl border border-[var(--border)] bg-white p-6">
              <h2 className="text-xl font-bold">ראיות והתאמות</h2>
              {identifiers.length ? (
                  ...
                  {identifiers.map((identifier) => (
                    ...
                    >
                      <p className="text-xs text-[var(--muted)]">
                        {identifierTypeLabel(identifier.type)}
                      </p>
                      <strong className="mt-1 block">{identifier.value}</strong>
                      <span className="mt-2 block text-xs text-amber-700">
                        התאמת חיפוש · טרם אומתה בתוכן העמוד
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <p className="mt-4 text-[var(--muted)]">
                  לא זוהתה התאמה למזהה קנוני. התוצאה נמצאה לפי שם הפרויקט או היישוב.
                </p>
              )}
              {finding.sourceUrl && (
                <div className="mt-6 flex gap-3">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex-1 rounded-lg bg-[var(--primary)] px-5 py-3 text-center font-semibold text-white"
                  >
                    פתיחת המקור
                  </a>
                  {canDownloadAsDocument && (
                    <button
                      ...
                      className="flex-1 rounded-lg border border-[var(--primary)] px-5 py-3 font-semibold text-[var(--primary)] hover:bg-blue-50"
                    >
                      הוספה לספריית המסמכים
                    </button>
                  )}
                </div>
              )}
            ...
            <aside className="rounded-xl border border-[var(--border)] bg-white p-6">
              ...
              <div className="mt-5 grid gap-3">
                <button
                  ...
                  disabled={saving}
                  className="rounded-lg bg-[var(--primary)] px-4 py-3 font-semibold text-white disabled:opacity-50"
                >
                  סימון כרלוונטי
                </button>
                <button
                  onClick={() => decide("irrelevant")}
                  disabled={saving}
                  className="rounded-lg border border-red-300 px-4 py-3 font-semibold text-red-700 disabled:opacity-50"
                >
                  סימון כלא רלוונטי
                </button>
              </div>
          ...
          {showDownloadDialog && finding?.sourceUrl && (
            <DocumentDownloadDialog
              ...
              onSuccess={() => {
                ...
                router.push(`/projects/${encodedSlug}/documents`);
              }}
            />
          )}
        </>
...
function FindingStatus({ finding }: { finding: FindingDetails }) {
  const label = finding.isRelevant === true
    ? "רלוונטי · דורש בדיקה"
    : finding.isRelevant === false
    ? "לא רלוונטי"
    : "ממתין לבדיקה";
  const style = finding.isRelevant === true
...
function parseMatchingIdentifiers(value: unknown): MatchingIdentifier[] {
  ...
  return value.filter((item): item is MatchingIdentifier =>
    Boolean(item) &&
    typeof item === "object" &&
...
function identifierTypeLabel(type: string) {
  return ({
    ...
    "housing-project-number": "פרויקט דיור",
    block: "גוש",
    parcel: "חלקה",
    lot: "מגרש",
    "plan-number": "תוכנית",
...
function sourceCategoryLabel(category: string) {
  return ({
    official: "מקור רשמי",
    municipal: "מקור עירוני",
    developer: "מקור היזם",
    private: "מקור פרטי",
    "user-upload": "מסמך משתמש"
