    ...
    <AppShell>
      ...
      <div className=”mt-6 grid gap-6 lg:grid-cols-[1fr_22rem]”>
          ...
          {identifiers.length ? (
            <div className=”mt-5 grid gap-3 sm:grid-cols-2”>
              {identifiers.map((identifier) => (
                <div key={`${identifier.type}:${identifier.value}`} className=”rounded-lg border border-[var(--border)] p-4”>
                  <p className=”text-xs text-[var(--muted)]”>{identifierTypeLabel(identifier.type)}</p>
                  <strong className=”mt-1 block”>{identifier.value}</strong>
                  <span className=”mt-2 block text-xs text-amber-700”>התאמת חיפוש · טרם אומתה בתוכן העמוד</span>
                </div>
              ))}
            </div>
          ) : (
            <p className=”mt-4 text-[var(--muted)]”>לא זוהתה התאמה למזהה קנוני. התוצאה נמצאה לפי שם הפרויקט או היישוב.</p>
          )}
          {finding.sourceUrl && (
            <div className=”mt-6 flex gap-3”>
              <a href={finding.sourceUrl} target=”_blank” rel=”noreferrer” className=”flex-1 rounded-lg bg-[var(--primary)] px-5 py-3 text-center font-semibold text-white”>
                פתיחת המקור
              </a>
              {canDownloadAsDocument && (
                <button onClick={() => setShowDownloadDialog(true)} className=”flex-1 rounded-lg border border-[var(--primary)] px-5 py-3 font-semibold text-[var(--primary)] hover:bg-blue-50”>
        ...
        <aside className=”rounded-xl border border-[var(--border)] bg-white p-6”>
          ...
          <p className=”mt-2 text-sm text-[var(--muted)]”>סימון כרלוונטי משאיר את הממצא במצב „דורש בדיקה”; הוא אינו מאמת אותו.</p>
          {error && <p className=”mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700”>{error}</p>}
          <div className=”mt-5 grid gap-3”>
            <button onClick={() => decide(“relevant”)} disabled={saving} className=”rounded-lg bg-[var(--primary)] px-4 py-3 font-semibold text-white disabled:opacity-50”>
              סימון כרלוונטי
            </button>
            <button onClick={() => decide(“irrelevant”)} disabled={saving} className=”rounded-lg border border-red-300 px-4 py-3 font-semibold text-red-700 disabled:opacity-50”>
              סימון כלא רלוונטי
            </button>
          </div>
        </aside>
