...
import { parseWinningMessage } from "@diratrack/domain";
...
interface FormState { message: string; messageSourceUrl: string; name: string; city: string; developer: string; lottery: string; housingProjectNumber: string; block: string; parcels: string; lot: string; }
interface MessageSummary { selectionPosition?: string; offeredApartments?: string; containsRegistrantNumber: boolean; extractedCount: number; }
const initialForm: FormState = { message: "", messageSourceUrl: "", name: "", city: "", developer: "", lottery: "", housingProjectNumber: "", block: "", parcels: "", lot: "" };

function NewProjectPage() {
  const router = useRouter();
  const [step, setStep] = useState<Step>("message");
  const [form, setForm] = useState(initialForm);
  const [extractedIdentifiers, setExtractedIdentifiers] = useState(() => new Set<string>());
  ...
  function continueFromMessage() {
    const parsed = parseWinningMessage(form.message);
    const projectNumber = parsed.housingProjectNumber?.value;
    setForm((current) => ({
      ...current,
      name: current.name || (projectNumber ? `פרויקט ${projectNumber}` : ""),
      city: parsed.city?.value ?? current.city,
      developer: parsed.developer?.value ?? current.developer,
      lottery: parsed.lotteryNumber?.value ?? current.lottery,
      housingProjectNumber: projectNumber ?? current.housingProjectNumber,
      messageSourceUrl: parsed.sourceUrl?.value ?? "",
    }));
    setExtractedIdentifiers(new Set([
      ...(parsed.lotteryNumber ? ["lottery"] : []),
      ...(parsed.housingProjectNumber ? ["housingProjectNumber"] : []),
    ]));
    setMessageSummary({
      selectionPosition: parsed.selectionPosition?.value,
      offeredApartments: parsed.offeredApartments?.value,
      containsRegistrantNumber: parsed.containsRegistrantNumber,
      extractedCount: [parsed.lotteryNumber, parsed.housingProjectNumber, parsed.city, parsed.developer, parsed.selectionPosition, parsed.offeredApartments, parsed.sourceUrl].filter(Boolean).length,
    });
    setStep("details");
  }
  function updateIdentifier(field: "lottery" | "housingProjectNumber", value: string) {
    update(field, value);
    setExtractedIdentifiers((current) => { const next = new Set(current); next.delete(field); return next; });
  }
  ...
  async function create() {
    ...
    const identifiers = [
      form.lottery && { type: "lottery-number", value: form.lottery, origin: extractedIdentifiers.has("lottery") ? "winning-message" : "manual", sourceUrl: extractedIdentifiers.has("lottery") ? form.messageSourceUrl || undefined : undefined },
      form.housingProjectNumber && { type: "housing-project-number", value: form.housingProjectNumber, origin: extractedIdentifiers.has("housingProjectNumber") ? "winning-message" : "manual", sourceUrl: extractedIdentifiers.has("housingProjectNumber") ? form.messageSourceUrl || undefined : undefined },
